   ...
   #>
    ...
    $list = foreach ($commandElementListItem in $commandElementList) {
        ...
        foreach ($commandElementListItemParameterItem in $commandElementListItem.Parameters) {
            switch ((Get-Command -Name $command.Name | Select-Object -ExpandProperty ParameterSets).Parameters | Where-Object { ($_.Name -eq $commandElementListItemParameterItem) -or ($_.Aliases.contains($commandElementListItemParameterItem)) }) {
                ...
                Default {
                    ...
                    foreach ($Char in $commandElementListItemParameterItem.ToCharArray()) {
                        ...
                        $count = ((Get-Command -Name $command.Name | Select-Object -ExpandProperty ParameterSets).Parameters.Name | Where-Object { $_ -like ('{0}*' -f $ShortestUniqueParameterMatch) }).Count
